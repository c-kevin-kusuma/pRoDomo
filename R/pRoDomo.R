                    ...
                    methods=list(
                      ...
                      task_list=function(project_id, limit=1,offset=0, df_output=TRUE){
                        ...
                        if( limit < 1 ){
                          ...
                          out <- httr::content(httr::GET(paste('https://',.self$domain,'/v1/projects/',project_id,'/tasks?offset=',offset,'&limit=',limit,sep=''),my_headers,query=list(sort='name',limit=limit,offset=offset)))
                        }
                      ...
                      task_create=function(project_id, list_id, task_def){
                        my_headers <- httr::add_headers(c('Content-Type'='application/json',Authorization=paste('bearer',.self$get_access(),sep=' ')))
                      ...
                      projects_all=function(limit=0,offset=0, df_output=TRUE){
                        "List all data sets"
                        my_headers <- httr::add_headers(c(Accept="application/json","Content-Type"="application/json",Authorization=paste('bearer',.self$get_access(),sep=' ')))
